...
const ProfileModel = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                const items = data.data.profileList.items;
                setProfile(items);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-r from-indigo-100 via-purple-100 to-pink-100 flex items-center justify-center p-6">
            {profile.map((user) => (
                <div key={user.email} className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-8 text-center">
                    ...
                    <p className="text-indigo-600 font-semibold text-lg">{user.designation}</p>
                    <p className="text-gray-500 mt-1">{user.email}</p>
                    <div className="border-t my-6"></div>
                    <div className="mt-6">
                        <h3 className="text-xl font-semibold text-gray-700 mb-2">About</h3>
                        <p className="text-gray-600 text-sm leading-relaxed">{user.biography?.plaintext}</p>
                    </div>
                </div>
            ))}
        </div>
